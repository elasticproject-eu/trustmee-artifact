...
impl Snp {
    ...
    fn fetch_vcek_from_kds(
        ...
        // Paper-eval: allow the caller to disable the on-disk VCEK cache so
        // each request must hit AMD KDS (eval 7).
...
pub fn evaluate(
    ...
    // Paper-eval: start of cert-chain verification.
    ...
    // Paper-eval: cert-chain phase ended; start of signature phase.
...
/// Paper-eval: when `WVC_EMIT_TIMING=1` is set in the wasm component env,
/// step timings are emitted to stderr as JSON events.
