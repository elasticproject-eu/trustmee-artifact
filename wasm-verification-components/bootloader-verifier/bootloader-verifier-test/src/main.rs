...
wasmtime::component::bindgen!({
    path: "../../tdx-verifier/tdx-verifier-component/wit",
    world: "verifier",
});

#[derive(Parser, Debug)]
#[command(name = "bootloader-verifier-test")]
struct Args {
    /// Path to the Wasm component (`.wasm`) built by `cargo build --target wasm32-wasip2 --release`.
    #[arg(long)]
    component: PathBuf,

    /// Path to full bootloader evidence JSON.
    #[arg(long, conflicts_with = "quote")]
    evidence_json: Option<PathBuf>,

    /// Path to a TDX quote (raw bytes), used to build evidence JSON.
    #[arg(long, required_unless_present = "evidence_json")]
    quote: Option<PathBuf>,

    /// Optional path to a CCEL (raw bytes), used to build evidence JSON.
    #[arg(long, conflicts_with = "evidence_json")]
    ccel: Option<PathBuf>,

    /// Optional PCCS/PCS base URL, used to build evidence JSON.
    #[arg(long, conflicts_with = "evidence_json")]
    pccs_url: Option<String>,

    /// Expected bootloader RTMR0 (96-char hex), used to build evidence JSON.
    #[arg(long, conflicts_with = "evidence_json")]
    expected_rtmr0_hex: Option<String>,

    /// Expected bootloader RTMR1 (96-char hex), used to build evidence JSON.
    #[arg(long, conflicts_with = "evidence_json")]
    expected_rtmr1_hex: Option<String>,

    /// Base cache directory on the host; a fresh subdirectory is pre-opened to the guest as `cache/`.
    #[arg(long, default_value = ".bootloader-verifier-cache")]
    cache_dir: PathBuf,
...
fn unique_cache_dir(base: &Path) -> Result<PathBuf> {
    ...
    let dir = base.join(format!("bootloader-verifier-cache-{nanos:x}-{pid}-{seq}"));
    std::fs::create_dir_all(&dir).with_context(|| format!("create {}", dir.display()))?;
    Ok(dir)
}

fn build_evidence_from_args(args: &Args) -> Result<Vec<u8>> {
    if let Some(path) = args.evidence_json.as_ref() {
    ...
    let quote_path = args
        .quote
        .as_ref()
        .ok_or_else(|| anyhow!("--quote is required unless --evidence-json is used"))?;
    let quote =
        std::fs::read(quote_path).with_context(|| format!("read {}", quote_path.display()))?;
    ...
    let evidence_json = serde_json::json!({
        ...
        "expected_rtmr0": args.expected_rtmr0_hex,
        "expected_rtmr1": args.expected_rtmr1_hex,
    });
    serde_json::to_vec(&evidence_json).context("serialize evidence JSON")
}
...
fn main() -> Result<()> {
    ...
    let evidence_bytes = build_evidence_from_args(&args)?;
