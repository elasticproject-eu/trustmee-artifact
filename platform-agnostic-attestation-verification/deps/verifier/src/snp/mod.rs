...
impl Snp {
    ...
    fn build_vcek_client(&self) -> reqwest_middleware::ClientWithMiddleware {
        // Paper-eval hook: skip the Moka HTTP cache entirely when
        // SNP_VCEK_DISABLE_CACHE=1 so every fetch goes to KDS.
        if matches!(
            std::env::var("SNP_VCEK_DISABLE_CACHE").ok().as_deref(),
...
impl Verifier for Snp {
    ...
    async fn evaluate(
        ...
        // Paper-eval: mark start of cert-chain verification.
        let cert_chain_start = Instant::now();
        ...
        // Paper-eval: cert-chain phase ended at the close of the match above.
        ...
        // Paper-eval: signature phase ends here.
        let signature_end = Instant::now();
        ...
        if snp_step_timing_enabled() {
            ...
            let pre_ms = cert_chain_start
                ...
                .as_secs_f64()
                * 1000.0;
            let post_ms = eval_end
                .saturating_duration_since(signature_end)
                .as_secs_f64()
                * 1000.0;
            let others_ms = pre_ms + post_ms;

            emit_snp_step_timing(cert_chain_ms, signature_ms, others_ms, total_ms);
        }
...
/// Emit the SNP step timing JSON event to stderr when SNP_STEP_TIMING_JSON=1.
fn snp_step_timing_enabled() -> bool {
    matches!(
        std::env::var("SNP_STEP_TIMING_JSON").ok().as_deref(),
        Some("1") | Some("true") | Some("yes")
    )
}

fn emit_snp_step_timing(cert_chain_ms: f64, signature_ms: f64, others_ms: f64, total_ms: f64) {
    let mode = std::env::var("SNP_TIMING_MODE").unwrap_or_else(|_| "native".to_string());
