...
pub async fn ecdsa_quote_verification(quote: &[u8]) -> anyhow::Result<Map<String, Value>> {
    ...
    // Paper-eval: measure the collateral fetch wall time. Emitted as a JSON
    // event to stderr when AS_VERIFICATION_TIMING_JSON=1 so eval_latency.py
    // can subtract it from the E2E latency (eval 1, native TDX).
    ...
    emit_collateral_timing_for_tee(
        quote_timing_tee(quote),
        "native",
        collateral_start.elapsed(),
    );
...
/// Emit as_tdx_collateral_timing JSON line to stderr when
/// AS_VERIFICATION_TIMING_JSON=1.
pub fn emit_collateral_timing(mode: &str, elapsed: Duration) {
    emit_collateral_timing_for_tee("Tdx", mode, elapsed);
}

pub fn emit_collateral_timing_for_tee(tee: &str, mode: &str, elapsed: Duration) {
    let enabled = matches!(
        std::env::var("AS_VERIFICATION_TIMING_JSON").ok().as_deref(),
...
fn quote_timing_tee(quote: &[u8]) -> &'static str {
    if quote
        .get(4..8)
        .map(|tee_type| tee_type == [0x81, 0x00, 0x00, 0x00].as_slice())
        .unwrap_or(false)
    {
        "Tdx"
    } else {
