...
async fn verify_evidence(
    ...
    // Paper-eval: optionally route through the pure-Rust dcap-qvl crate (same
    // library the wasm TDX verifier uses) for eval 6's native-vs-wasm comparison.
    let custom_claims = {
        #[cfg(feature = "tdx-verifier-dcap-qvl")]
        {
            if crate::intel_dcap_qvl::use_dcap_qvl_native() {
                crate::intel_dcap_qvl::ecdsa_quote_verification_via_dcap_qvl(quote_bin.as_slice())
                    .await?
            } else {
                ecdsa_quote_verification(quote_bin.as_slice()).await?
            }
        }
        #[cfg(not(feature = "tdx-verifier-dcap-qvl"))]
        {
            ecdsa_quote_verification(quote_bin.as_slice()).await?
        }
    };
