...
async fn verify_evidence(
    ...
    let custom_claims = {
        #[cfg(feature = "sgx-verifier-dcap-qvl")]
        {
            if crate::intel_dcap_qvl::use_sgx_dcap_qvl_native() {
                crate::intel_dcap_qvl::sgx_ecdsa_quote_verification_via_dcap_qvl(
                    quote_bin.as_slice(),
                )
                .await?
            } else {
        ...
        #[cfg(not(feature = "sgx-verifier-dcap-qvl"))]
        {
            ecdsa_quote_verification(quote_bin.as_slice()).await?
        }
    };
