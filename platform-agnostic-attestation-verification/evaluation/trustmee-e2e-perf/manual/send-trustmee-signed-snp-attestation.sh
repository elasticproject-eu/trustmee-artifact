...
usage() {
    cat <<'EOF'
Usage: send-trustmee-signed-snp-attestation.sh [--port <port>] [--component <path>] [--evidence <path>] [--output-dir <dir>] [--unstapled]

Builds the TrustMee EAT + CMW request for the sample signed SNP verifier component,
sends it to /attestation, and saves the generated artifacts plus the returned token.

Start `restful-as` with `--component-trust-store` pointing at the matching trust
store before using this script.

Defaults:
  --port       8080
  --component  test_data/trustmee-lib/signature-demo/snp_verifier_component.signed.wasm
  --evidence   test_data/trustmee-lib/snp_evidence.json
  --output-dir evaluation/trustmee-e2e-perf/results/manual/signed-snp-request-<timestamp>
  --unstapled  omit CMW `verifier` endorsement and rely on component cache / OCI
EOF
}

PORT=8080
COMPONENT_PATH="$DEFAULT_SIGNED_SNP_WASM_COMPONENT_PATH"
EVIDENCE_PATH="$DEFAULT_SNP_EVIDENCE_PATH"
OUTPUT_DIR="$DEFAULT_RESULTS_ROOT/manual/signed-snp-request-$(timestamp_utc)"
...
RESPONSE_PATH="$OUTPUT_DIR/attest-trustmee-signed-snp-token.txt"
HEADERS_PATH="$OUTPUT_DIR/attest-trustmee-signed-snp-response.headers.txt"
...
request: wasm-snp-signed
port: $PORT
component: $COMPONENT_PATH
evidence: $EVIDENCE_PATH
recommended_component_trust_store: $DEFAULT_SIGNED_SNP_TRUST_STORE_PATH
...
if [[ "$HTTP_CODE" != "200" || ! -s "$RESPONSE_PATH" ]]; then
    die "signed attestation request failed with http_code=$HTTP_CODE, see $SUMMARY_PATH"
fi
